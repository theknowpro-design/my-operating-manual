...
function drawFooter(doc, layout, branding, pageNumber) {
  ...
  doc.text(branding.footerText || 'Generated by Money Maker App', left, pageHeight - bottom + 24);
...
async function renderCoverPage(doc, layout, branding, content, logoAsset) {
  try {
    ...
    const safeTitle = content?.title || 'Profit Engine Plan';
...
async function generatePdfDocumentFromSections(input = {}, JsPDFCtor) {
  ...
  content.title = content.title || 'Profit Engine Plan';
...
export async function generatePdfDocument(input = {}) {
  ...
  try {
    ...
    await doc.html(html, {
      callback: () => {
        doc.save(input.filename || 'profit-engine.pdf');
      },
      html2canvas,
    });
    return true;
